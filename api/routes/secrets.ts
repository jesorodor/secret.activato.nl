...
const app = new Hono<{
    ...
    .post('/', zValidator('json', createSecretsSchema), async (c) => {
        try {
            ...
            // Activato policy: expiration and views are server-controlled.
            // Ignore client-supplied values and enforce the admin config.
            ...
            const data: SecretCreateData = {
                ...
                // Views locked to a single view (Activato policy)
                views: 1,
                // Burn after time removed (Activato policy)
                isBurnable: false,
